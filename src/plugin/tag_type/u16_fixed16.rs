...
pub fn type_u16_fixed16_dup(_handler: &TagTypeHandler, ptr: &dyn Any, _n_items: usize) -> Result<Box<dyn Any>> {
    match ptr.downcast_ref::<Vec<f64>>() {
        None => Err("Invalid object to duplicate with type_u16_fixed16_dup".into()),
        Some(value) => Ok(Box::new(value.clone())),
    }
}

pub fn type_u16_fixed16_free(_handler: &TagTypeHandler, ptr: Box<dyn Any>) {
    drop(ptr);
}
